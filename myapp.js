...
async function getWFore() {
  return fetch(WEATHERFORECASTURL).then((response) => response.json());
}
...
async function main() {
  ...
  try {
    ...
    //do this when encountering error
    console.error(err);
...
function generateHeader(Odata) {
  ...
  rainBlk.innerHTML =
    rainIcon + "<span id='rain'>" + vol + "</span><small>mm</small>";
  ...
  lastUpdateTimeBlk.innerHTML = `Last Update: ${Odata.updateTime.substring(
    st,
    st + 5
  )}`;
...
function generateTempBlk(Odata) {
  ...
  let tselect =
    '<label>Select the location</label><select name="temp" id="temp-reg">' +
    toptions +
    "</select>";
  ...
  document.getElementById("temp-reg").addEventListener("change", (evt) => {
    ...
    document.getElementById(
      "tblock"
    ).innerHTML = `<span class='ltemp'>${value}</span><span class="sup2">°C</span>`;
  });
}
...
function generateWForecast(WFore) {
  ...
  for (let day of WFore.weatherForecast) {
    ...
    let date =
      parseInt(day.forecastDate.substring(6), 10) +
      "/" +
      parseInt(day.forecastDate.substring(4, 6), 10);
